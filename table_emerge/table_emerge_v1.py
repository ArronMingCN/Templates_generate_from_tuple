...
def run_table():
    ...
    for key in dict_china.keys():
        ...
        if value2 == "":
            value2 = "nothing"
        row_value = get_last_row_ele(key)
        #fout_table_china.write(row_value + "||" + value1 + "||" + value2 + "|||")
    ...
    for key in dict_overall.keys():
        ...
        if value2 == "":
            value2 = "nothing"
        row_value = get_last_row_ele(key)
        #fout_table_overall.write(row_value+"||"+value1+"||"+value2+"|||")
        table_contents += (row_value+"||"+value1+"||"+value2+"|||")
    table_contents = table_contents[:-3]
...
fout_table_overall = open("table_overall.txt","w+",encoding="utf-8")
# 全局变量，单表内容信息,表间用||||隔开
table_contents=""
# 全局变量，合并后的内容
...
if __name__ == "__main__":
    ...
    print(graph_data)
    # dicts = {'abc':"aaa",'bcd':'bbbbbbb'}
    # if len(['1' for key in dicts.keys() if 'a' in key]):
    #     print("test")
